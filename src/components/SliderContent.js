    ...
    return (
        <div className="relative flex items-center  container lg:my-4 w-[90%] h-[40%]" id="Overview">
            ...
            <div className="relative max-sm:h-[100%]  max-sm:flex max-sm:flex-col max-sm:justify-between max-sm:items-centre z-10 w-full md:w-1/2 text-black sm:w-[1/1] text-left max-sm:pb-8  ">
                {/* Content */}
                <div>
                    <h1 className="text-4xl font-semibold max-md:mt-4 mb-4 md:w-4/5 max-sm:text-center">{props.title} <span className="text-blue">{props.subtitle}</span></h1>
                    <p className={`mb-8 lg:mb-4 md:w-4/5 text-lg-normal leading-7 md:leading-6 max-sm:text-center md:text-left`}>
                        {props.description}
                    </p>
                </div>

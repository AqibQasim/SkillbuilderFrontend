...
function Window({ children, name }) {
  ...
  return createPortal(
    <div className="bg-black fixed inset-0 z-50 h-full w-full bg-opacity-10 backdrop-blur-sm transition-all duration-500">
      <div className="fixed left-1/2 top-1/2 max-w-lg -translate-x-1/2 -translate-y-1/2 transform rounded-2xl border border-border_gray bg-white p-6 shadow-sm transition-all duration-500">
        <button

...
const InstructorVideos = ({ onNext, onPrev }) => {
  ...
  return (
    <div>
      <div className="accordion mt-8 rounded-md border-2 border-[#BBBBBB] px-4 py-2">
            ...
            >
              <div className="border-[#BBBBBB] py-5">
                <div className="flex w-full flex-col items-center">
                  ...
                  {Object.keys(showVideos).map((key) =>
                    showVideos[key] && key.startsWith(`${moduleIndex}-`) ? (
                      <video key={key} className="h-96 w-full" controls>
                        <source
                          src={item.videos[parseInt(key.split("-")[1], 10)].url}
                          type={
                            item.videos[parseInt(key.split("-")[1], 10)].file
                              .type
                          }
                        />

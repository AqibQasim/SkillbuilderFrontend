      ...
      <div className="w-[100%] flex justify-between items-center bg-white max-sm:pt-10">
        <div className="w-[100%] pt-5 pb-10 px-10">
          <div className="flex justify-between max-sm:flex-col gap-3 w-[100%] ">
            ...
            <div>
              <DropDown text="Most Relevant" />
            </div>
          </div>

          <div className="text-base max-sm:mt-5">{total} Total</div>
          <div className="w-[100%]">
            <ReviewRow barwidth={`${(counts.five / total) * 100}% `} number={5} />
            <ReviewRow barwidth={`${(counts.four / total) * 100}% `} number={4} />
            <ReviewRow barwidth={`${(counts.three / total) * 100}% `} number={3} />
            <ReviewRow barwidth={`${(counts.two / total) * 100}% `} number={2} />
            <ReviewRow barwidth={`${(counts.one / total) * 100}% `} number={1} />
          </div>
        </div>
      </div>
    </div>

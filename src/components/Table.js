...
function Row({ children, onClick }) {
  ...
  return (
    <div
      onClick={onClick}
      className={`grid items-center capitalize tracking-wider grid-cols-[${columns}] ${columns} text-black group cursor-pointer items-center gap-4 p-4 tracking-tight transition-all duration-200 hover:bg-white`}
    >
      {children}
    </div>
  );

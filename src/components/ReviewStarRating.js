...
const ReviewStarRating = ({ rating, setRating }) => {
  ...
  return (
    <div className="flex flex-wrap gap-1 sm:gap-4 md:gap-5 lg:gap-6 xsm:gap-3">
      {[...Array(5)].map((_, i) => (
        <div
          key={i}
        ...
        >
          <FontAwesomeIcon
            ...
            className={
              i < rating
                ? "text-sm text-yellow-500 sm:text-2xl md:text-4xl lg:text-6xl xsm:text-lg"
                : "text-sm text-gray-200 sm:text-2xl md:text-4xl lg:text-6xl xsm:text-lg"
            }
          />
        </div>
      ))}
